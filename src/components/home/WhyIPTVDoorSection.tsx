      ...
      <div className="grid gap-5 md:grid-cols-3">
        {primaryFeatures.map((feature, index) => {
          ...
          return <motion.article key={feature.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`flex min-h-[285px] flex-col rounded-2xl border p-6 border-[#36a9ff]/30 bg-[#051f33]/80 backdrop-blur-sm shadow-[0_0_20px_rgba(54,169,255,0.15)] hover:border-[#36a9ff] hover:shadow-[0_0_30px_rgba(54,169,255,0.3)] transition-all duration-300`}>
            ...
            <div className="mt-auto flex flex-wrap gap-2 pt-5">{feature.tags.map(tag => <span key={tag} className="rounded-full border border-white/10 bg-[#051f33] px-2.5 py-1 text-[9px] font-bold tracking-wide text-gray-300">{tag}</span>)}</div>
          </motion.article>;
        })}
      </div>

      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {benefits.map((benefit, index) => { const Icon = benefit.icon; return <motion.div key={benefit.title} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.25 + index * 0.07 }} className="flex min-h-[112px] gap-3 rounded-xl border border-[#36a9ff]/20 bg-[#051f33]/80 backdrop-blur-sm p-5 hover:border-[#36a9ff]/50 transition-colors"><Icon className={`h-5 w-5 shrink-0 text-[#36a9ff]`} /><span><span className="block text-sm font-bold text-white">{benefit.title}</span><small className="mt-1 block text-[11px] leading-4 text-gray-400">{benefit.text}</small></span></motion.div>; })}
      </div>
    </div>
  </section>;
}
